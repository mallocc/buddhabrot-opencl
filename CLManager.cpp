...
std::string CLManager::getOpenCLErrorString(cl_int errorCode) {
	switch (errorCode) {
	case CL_SUCCESS:
		return "CL_SUCCESS";
...
bool CLManager::__CheckError(cl_int err, const char* file, int line) {
	if (err != CL_SUCCESS) {
...
std::string CLManager::GetPlatformName(cl_platform_id id)
{
	size_t size = 0;
...
std::string CLManager::GetDeviceName(cl_device_id id)
{
	size_t size = 0;
...
cl_program CLManager::CreateProgram(const std::string& source, cl_context context, cl_int* error)
{
...
std::string CLManager::LoadKernel(const char* name)
{
...
bool CLManager::execute(size_t globalSize)
{
	cl_event eve;
...
bool CLManager::setKernelArgs(std::vector<CLKernelArg*> args)
{
